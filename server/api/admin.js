...
async function load() {
  ...
  try {
    ...
    for (const row of result.items) {
      ...
      for (const value of [row.reference + '\n' + timestamp(row.createdAt), row.name || row.nameMajor, states[kind][row.status], row.device || row.volunteerNames]) {
        const td = document.createElement('td'); td.textContent = value; tr.append(td);
      }
      const cell = document.createElement('td'), button = document.createElement('button');
      button.type = 'button'; button.textContent = '查看 / 处理'; button.addEventListener('click', () => open(kind, row.id)); cell.append(button); tr.append(cell); $('#rows').append(tr);
    }
...
$('#kind').addEventListener('change', () => { options($('#filter-status'), $('#kind').value, true); page = 1; load(); });
...
async function loadSettings() {
  try {
    ...
    $('#closed-message').value = settings.closedMessage; $('#settings-fields').disabled = false;
...
$('#settings').addEventListener('submit', async event => {
  ...
  try { settings = await api('settings', { method: 'PATCH', body: JSON.stringify(value) }); message('接收设置已保存。'); }
  catch (error) { message(error.message, true); }
